...
func largestProduct1(mx [][]int) int {
	...
	for y := 0; y < 20; y++ {
		...
		for x := 0; x < 20-3; x++ {
			...
			if product > largestProduct {
				...
				// pf("H: (%d, %d): %d * %d * %d * %d = %d\n", x, y, mx[y][x], mx[y][x+1], mx[y][x+2], mx[y][x+3], product)
			}
		}
	}
	...
	for y := 0; y < 20-3; y++ {
		...
		for x := 0; x < 20; x++ {
			...
			if product > largestProduct {
				largestProduct = product
				// pf("V: (%d, %d): %d * %d * %d * %d = %d\n", x, y, mx[y][x], mx[y+1][x], mx[y+2][x], mx[y+3][x], product)
			}
		}
	}
	...
	for y := 0; y < 20-3; y++ {
		...
		for x := 0; x < 20-3; x++ {
			...
			if product > largestProduct {
				largestProduct = product
				// pf("B: (%d, %d): %d * %d * %d * %d = %d\n", x, y, mx[y][x], mx[y+1][x+1], mx[y+2][x+2], mx[y+3][x+3], product)
			}
		}
	}
...
func largestProduct2(mx [][]int) int {
...
func main() {
	...
	largestProduct1(Matrix)
	largestProduct2(Matrix)
	//p(matrix[0][2])
